...
async def on_message(message):
	...
	message_date = int(math.floor(time.time()))
	...
	if len(content) > 0 or len(attachments) > 0:
		...
		db.write_msg(database, message_id, message_date, content, guild_name, guild_channel, len(attachments) > 0)
		
		for attachment in attachments:
			(attachment_id, file_name, file_size) = img.save(attachment.url)
			db.write_attachment(database, attachment_id, file_name, file_size, message_id)
